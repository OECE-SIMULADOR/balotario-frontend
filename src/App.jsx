...
const queryClient = new QueryClient({
  defaultOptions: {
    ...
    mutations: { // <-- NUEVA SECCIÓN
      retry: (failureCount, error) => {
        // No reintentar en errores de cliente (4xx), solo en errores de servidor (5xx) o de red
        if (error.response && error.response.status >= 400 && error.response.status < 500) {
          return false;
        }
        // Reintentar hasta 2 veces más (3 intentos en total)
